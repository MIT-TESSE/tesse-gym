...
'''
How to build SceneGraph object?
for each layer:
	for each node in layer:
		make node object
		add to node list
	for each edge in layer:
		make edge object
		add to object list
		(add source layer and target layer attributes in edge class?)
...
def load_graph(graph):
	...
	for node in G.nodes:
		new_node = ESDFNode()
		new_node.node_id = node.id.value
		new_node.node_type = node.
		new_node.centroid = node.position
		g.add_node(new_node) 
	
	for edge in G.edges:
		new_edge = SceneEdge()
		new_edge.start = edge.source
		new_edge.rel = None #?
		new_edge.end = edge.target
		if edge.weighted:
...
# Example of DSG binding 
path_to_dsg = "~/Downloads/dsg.json"
